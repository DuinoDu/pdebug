    ...
    for ind, roi in enumerate(roidb):
        ...
        if args.save_split is not None:
            if ind % args.save_split == 0 and ind > 0:
                ...
                savepkl = os.path.splitext(args.roidb)[0] + '_%d.pkl' % (cnt-1)
        ...
        new_imgfile = os.path.splitext(imgfile)[0] + '_small' + os.path.splitext(imgfile)[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--add_keypoints', dest='add_keypoints', action='store_true', help='copy lmks to keypoints')
    parser.add_argument('--save_split', default=None, type=int, help='save pkl in splitted file')
    parser.add_argument('--ignore_cache', dest='ignore_cache', action='store_true', help='write image even if it already exist')

...
def main(args):
    ...
    with open(args.cocoGt, 'r') as fid:
        ...
        for ind, ann_img in enumerate(coco['images']):
            ...
            if image_name not in results_dict:
                ...
                results_dict[image_name]['width'], results_dict[image_name]['height'] = fetch_image_wh(os.path.join(args.imgdir, image_name))
            ...
            anns = [x for x in coco['annotations'] if x[u'image_id'] == image_id]
            if len(anns) == 0:
                no_people += 1
                ...
                for ann in anns:
                    bbox = box_transform_inv(ann[u'bbox'])
                    bbox.extend([1.0])    # add bbox score
                    keypoints = ann[u'keypoints']
                    if not args.keep_allzero and np.count_nonzero(np.array(keypoints)) == 0:
                        continue
                    results_dict[image_name]['boxes'].append(bbox)
                    results_dict[image_name]['keypoints'].append(keypoints)
                    results_dict[image_name]['area'].append(ann[u'area'])
                    results_dict[image_name]['iscrowd'].append(ann[u'iscrowd'])
            if len(results_dict[image_name]['keypoints']) == 0:
                results_dict.pop(image_name)

    results = [{'image': os.path.join(args.imgdir, x),
                'width': results_dict[x]['width'],
                'height': results_dict[x]['height'],
                'boxes': np.array(results_dict[x]['boxes'], dtype=np.float32),
                'keypoints': np.array(results_dict[x]['keypoints'], dtype=np.float32),
                'gt_classes': np.ones((len(results_dict[x]['boxes']),), dtype=np.int32),
                'flipped': False,
                'rotation': False,
                'upper_body': False,
                'degree': 0,
    ...
    print('have people: %d' % len([x for x in results if x['boxes'].shape[0] != 0]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cocoGt', default='', type=str, help='cocoGt', required=True)
    parser.add_argument('--imgdir', default='', type=str, help='imgdir', required=True)
    parser.add_argument('--output', default=None, type=str, help='output roidb pkl file')
    parser.add_argument('--keep_allzero', dest='keep_allzero', action='store_true', help='keep all zeros in keypoints, used when evaluate coco5k')

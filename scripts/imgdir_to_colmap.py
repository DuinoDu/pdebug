...
def main(
    ...
    matcher: Optional[str] = typer.Option("sequential", help="select which matcher colmap should use. sequential for videos, exhaustive for adhoc images"),
    ...
    camera_params: Optional[str] = typer.Option("", help="intrinsic parameters, depending on the chosen model.  Format: fx,fy,cx,cy,dist"),
    ...
    do_system(f"colmap feature_extractor --ImageReader.camera_model {camera_model} --ImageReader.camera_params \"{camera_params}\" "
    ...
    do_system(f"colmap {matcher}_matcher --SiftMatching.guided_matching=true --database_path {db} --SiftMatching.gpu_index 0 ")

    sparse = os.path.join(output, "sparse")
    os.makedirs(sparse, exist_ok=True)
    do_system(f"colmap mapper --database_path {db} --image_path {imgdir} --output_path {sparse}")
    ...
    do_system(f"colmap model_converter --input_path {sparse}/0 --output_path {sparse}/0 --output_type TXT")

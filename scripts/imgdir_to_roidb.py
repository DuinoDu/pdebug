...
def main(
    ...
    add_keypoints: Optional[bool] = typer.Option(False, help="add fake keypoints"),
    num_keypoints: Optional[int] = typer.Option(17, help="fake keypoints number"),
    ...
    roidb = []
    imgfiles = sorted([os.path.join(imgdir, x) for x in sorted(os.listdir(imgdir)) if x.endswith(ext)])

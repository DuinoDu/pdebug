...
def main(args):
    ...
    imgfiles = sorted([os.path.join(args.imgdir, x) for x in sorted(os.listdir(args.imgdir)) if os.path.splitext(x)[1] in imgext])
    ...
    if args.imglist is not None:
        ...
        for ind, item in enumerate(imgnames):
            ...
            cmd = '%s %s %s' % (sh, os.path.join(args.imgdir, item), args.output)
            os.system(cmd)
    ...
    if args.roidb is not None:
        ...
        for ind, roi in enumerate(roidb):
            ...
            if args.to_jpg and 'jpg' not in os.path.splitext(imgname)[1].lower():
    ...
    if args.random_num is not None:
        assert args.random_num < len(imgfiles), "%d < %d" % (args.random_num, len(imgfiles))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--imgdir', default=None, type=str, help='imgdir', required=True)
    parser.add_argument('--link', dest='link', action='store_true', help='if choose, using ln instead of cp')
    ...
    parser.add_argument('--to_jpg', dest='to_jpg', action='store_true', help='convert to jpg format')
    parser.add_argument('--cleanimage', dest='cleanimage', action='store_true', help='clean images, rm bad images')
    parser.add_argument('--flip', dest='flip', action='store_true', help='if hflip image')
    parser.add_argument('--random_num', default=None, type=int, help='random sample from imgdir to output')

...
def main(
    ...
    if len(imgfiles) != len(depfiles):
        print(f"len of images {len(imgfiles)} != len of depth files({len(depfiles)})")
    ...
    if sync_file:
        ...
        sync_info = [line.strip().split(" ") for line in open(sync_file, "r").readlines()]
        imgfiles = [os.path.join(imgdir, os.path.basename(l[0])) for l in sync_info]
        depfiles = [os.path.join(depdir, os.path.basename(l[3])) for l in sync_info]
    ...
    for imgfile, depfile in zip(imgfiles, depfiles):
        ...
        vis_depth = draw.depth(depth, image, vertical=False, max_depth=1000, cmap=cmap, hist=True)

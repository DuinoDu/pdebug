...
from utils import rotate_image, rotate_boxes, rotate_kps
import cv2
import numpy as np
...
def main(args):
    ...
    for ind, roi in enumerate(roidb):
        ...
        save_imgfile = os.path.join(output_imgdir, os.path.basename(roi['image']))
        ...
        boxes_r = rotate_boxes(roi['boxes'], args.angle, src_img_shape, dst_img_shape)
        ...
        roi['boxes'] = boxes_r
        kps_r = rotate_kps(roi['keypoints'], args.angle, src_img_shape, dst_img_shape)
        roi['keypoints'] = kps_r

...
def main(args):
    ...
    for path, dirs, files in os.walk(args.imgdir, followlinks=True):
        images.extend([os.path.join(path, x) for x in files if x.endswith(args.ext)])
    assert args.rename_method in ('keepname', 'concat', 'index'), "Unknown rename method: %s" % args.rename_method
    ...
    if args.roidb is not None:
        ...
        with open(args.roidb, 'rb') as fid:
            ...
            try:
                imglist = [os.path.basename(roi['image_name']) for roi in roidb]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Collect images from dir, recursively')
    parser.add_argument('--imgdir', default=None, type=str, help='input image dir, recursively', required=True)
    parser.add_argument('--rename_method', default='keepname', type=str, help='image rename method, keepname | concat | index')
    ...
    parser.add_argument('--copy', dest='copy', action='store_true', help='if copy image, default is link')
    parser.add_argument('--depth', default=1, type=int, help='concat dir depth, only used when rename_method is concat')
    parser.add_argument('--imglist', default=None, type=str, help='choose imglist')
    parser.add_argument('--roidb', default=None, type=str, help='choose imglist from roidb')
    parser.add_argument('--min', default=-1, type=int, help='start image index')

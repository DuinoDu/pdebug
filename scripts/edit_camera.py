...
def main(
    ...
    if NerfCamera.is_valid(input_json):
        typer.echo(typer.style(f"camera type: nerf camera", fg=typer.colors.GREEN))
        camera = NerfCamera.fromfile(input_json)
        poses = [np.array(f["transform_matrix"], dtype=np.float32)
                 for f in camera.frames]
    elif Open3dCameraTrajectory.is_valid(input_json):
        typer.echo(typer.style(f"camera type: open3d camera", fg=typer.colors.GREEN))
        camera = Open3dCameraTrajectory.fromfile(input_json)
        poses = [np.array(p["extrinsic"], dtype=np.float32).reshape(4, 4).transpose()
    ...
    # apply rotation
    rot_vec = np.asarray([rotation_x, rotation_y, rotation_z]) / 180. * np.pi
    rotation = [R.from_matrix(pose[:3, :3]).as_euler('xyz') + rot_vec for pose in poses]

...
if VISDOM_INSTALLED:
    import visdom
    from visdom.server import start_server, download_scripts
...
class WebEngine(ABC):
    def __init__(
        ...
        if self.serve_first:
            self.p = Process(target=self.serve,
                        kwargs=serve_kwargs if serve_kwargs else {})
            self.p.start()
...
class ImageEngine(WebEngine):
    ...
    def __init__(
        ...
        if VISDOM_INSTALLED:
            print("Found visdom installed, please make sure "
    ...
    def add_visdom_image(
            image: Tensor,
            vis: Optional[visdom.Visdom] = None,

...
from pdebug.utils.env import TIMM_INSTALLED, TORCH_INSTALLED
...
__all__ = ["repvit_m1_1", "repvit_m1_5", "repvit_m2_3"]
...
class ConvNorm(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "c",
            nn.Conv2d(
                in_dim, out_dim, ks, stride, pad, dilation, groups, bias=False
            ),
        )
...
class RepVggDw(nn.Module):
    def __init__(self, ed, kernel_size, legacy=False):
        super().__init__()
        self.conv = ConvNorm(
            ed, ed, kernel_size, 1, (kernel_size - 1) // 2, groups=ed
        )
        # self.conv = ConvNorm(ed, ed, kernel_size, 1, (kernel_size - 1) // 2)
        if legacy:
            # self.conv1 = ConvNorm(ed, ed, 1, 1, 0)
            self.conv1 = ConvNorm(ed, ed, 1, 1, 0, groups=ed)
            # Make torchscript happy.
            self.bn = nn.Identity()
        else:
            self.conv1 = nn.Conv2d(ed, ed, 1, 1, 0, groups=ed)
            # self.conv1 = nn.Conv2d(ed, ed, 1, 1, 0)
...
class RepViTBlock(nn.Module):
    def __init__(
        self, in_dim, mlp_ratio, kernel_size, use_se, act_layer, legacy=False
    ):
...
class RepVitStage(nn.Module):
    def __init__(
        ...
        for _ in range(depth):
            blocks.append(
                RepViTBlock(
                    out_dim, mlp_ratio, kernel_size, use_se, act_layer, legacy
                )
            )
            # use_se = not use_se
...
class RepVit(nn.Module):
    """
    Adapted from official impl at https://github.com/THU-MIG/RepViT
    """
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            stride *= stage_stride
            resolution = tuple(
                [(r - 1) // stage_stride + 1 for r in resolution]
            )
            self.feature_info += [
                dict(
                    num_chs=embed_dim[i],
    ...
    def reset_classifier(
        ...
        if global_pool is not None:
            self.global_pool = global_pool
        self.head = (
            RepVitClassifier(self.embed_dim[-1], num_classes, distillation)
            if num_classes > 0
            else nn.Identity()
        )

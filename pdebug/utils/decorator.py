...
def line_prof(decorated_=None, block=True, output='lp_output.txt.py'):
    ...
    def _decorator(func):
        ...
        def __decorator(*args, **kwargs):
            ...
            fid.close()
            if block:
                print('exit when line_prof, disable by exit=False in line_prof')
...
def has_process_id(func):
    ...
    if "process_id" in args:
        assert args.index("process_id") == 0, "`process_id` can only be the first args"
        return True
    else:
        return False


def mp(decorated_=None, nums=None):
    """ make your func run in multi-process

    Parameters
    ----------
    nums: int
        process nums, default is 4
    no_return: bool
        True: set timeout to 10ms
        False: block until all process finish
    ...
    def actual_decorator(func):
        ...
        def inner(*args, **kwargs):
            ...
            # slice input list
            process_nums = 4 if nums is None else nums
            ...
            process_pool = [_mp.Process(target=compute_func, args=(in_q, out_q))
            ...
            try:
                for idx, data_list in enumerate(data_lists):
                    ...
                    # inject "project_id" into `input_args`
                    if has_process_id(func):

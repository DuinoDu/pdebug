...
class Line:
    ...
    unit_vec: [x, y]
    """
    def __init__(self, p0=None, p1=None, unit_vec=None,
                 angle=None, scale=None, reverse=False, israd=False):
        self.p0 = p0
        self.p1 = p1
        if reverse:
            if p0 is not None:
        ...
        self._angle = angle / np.pi * 180. if israd else angle
        if self._angle is not None and unit_vec is None:
            vec_x = 1.0 * np.cos(self._angle / 180. * np.pi)
            vec_y = 1.0 * np.sin(self._angle / 180. * np.pi)
    ...
    def __call__(self, x=None, y=None, x1=None, y1=None, with_direction=False):
        ...
        if self.unit_vec is not None:
            ...
            tan_angle = (self.p1[0] - self.p0[0]) / (self.p1[1] - self.p0[1] + self.eps)
        else:
    ...
    def draw(self, _img, color=(0,255,0), thickness=2, bbox=None, with_direction=False):
        if self.unit_vec is not None:
            # compute border
            if bbox is not None:
                left = bbox[0]

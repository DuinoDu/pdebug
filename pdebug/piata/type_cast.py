...
__all__ = ['points_to_keypoints', 'keypoints_to_points',
           'points_to_boxes', 'boxes_to_points']


BAD_POINT_VALUE = -1


def points_to_keypoints(points: np.ndarray, vis: Optional[int] = 2) -> np.ndarray:
...
def points_to_boxes(points: np.ndarray, point_size: int, layout="x1y1wh") -> np.ndarray:
    ...
    W = X2 - X1
    H = Y2 - Y1
    boxes = np.concatenate((X1[:, None], Y1[:, None], W[:, None], H[:, None]), axis=1)
    return boxes

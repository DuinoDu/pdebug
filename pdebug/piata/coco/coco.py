...
class COCO(MSCOCO):
    ...
    def getAnnIds(self, imgIds=[], catIds=[], areaRng=[], iscrowd=None, actionIds=[]):
        """
        ...
        if len(imgIds) == len(catIds) == len(areaRng) == len(actionIds) == 0:
            ...
            anns = anns if len(actionIds)  == 0 else [ann for ann in anns if ann['action_id'] in actionIds]
        if not iscrowd == None:
        ...
        if len(imgIds) == len(catIds) == len(videoIds) == len(frameIds) == 0:
            ...
            for i, videoId in enumerate(videoIds):
                if i == 0 and len(ids) == 0:
                    ...
                    ids &= set(self.videoToImgs[videoId])
            ids = ids if len(frameIds)  == 0 else \
                  [_id for _id in ids if self.imgs[_id]['frame_id'] in frameIds]
        return list(ids)
    ...
    def getVideoIds(self, videoIds=[], videoNms=[]):
        ...
        if len(videoIds) == len(videoNms) == 0:
            ...
            videos = self.dataset['videos']
            videos = videos if len(videoIds) == 0 else [video for video in videos if video['id']        in videoIds]
            videos = videos if len(videoNms) == 0 else [video for video in videos if video['name'] in videoNms]
    ...
    def getActionIds(self, actionIds=[], actionNms=[]):
        ...
        if len(actionIds) == len(actionNms) == 0:
            ...
            actions = self.dataset['actions']
            actions = actions if len(actionIds) == 0 else [action for action in actions if action['id']   in actionIds]
            actions = actions if len(actionNms) == 0 else [action for action in actions if action['name'] in actionNms]

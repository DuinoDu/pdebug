...
class Input:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs['name'] in ROIDB_REGISTRY:
            ...
            if kwargs['name'] == 'coco':
                raise ValueError(f"please import piata.coco before using coco")
            else:
                raise ValueError(f"Unvalid name: {name}")
    ...
    def get_roidb(self, as_dict=False, keepname=False):
        ...
        if len(self._roidb) > len(set([x['image_name'] for x in self._roidb])):
            # change to img-wise
        ...
        if as_dict:
            ...
            if keepname:
                ...
                roidb_dict = {os.path.basename(x['image_name']): x for x in self._roidb}    # noqa
            return roidb_dict
        else:
            return self._roidb
    ...
    def get_tracklet(self, action_dict):
        ...
        for line in roidb:
            ...
            try:
                for trackid, bbox, kps in zip(line['track_id'], line['boxes'], line['keypoints']):  # noqa

                    imgname_to_trackid_to_kps.setdefault(
                        imgname, {}).setdefault(trackid, []).append(kps)
                    imgname_to_trackid_to_bbox.setdefault(
                        imgname, {}).setdefault(trackid, []).append(bbox)
            except:
                continue
            ...
            for i in range(len(one_act_roidb['image_names'])):
                ...
                for trackid, action_label in zip(track_ids, action_labels):
                    try:
                        ...
                        tracklets.update(trackid, bbox, kps, [action_label, 1.0])  # noqa
        ...
        for roi in roidb:
            if 'boxes' in roi:
                ...
                if num_boxes is None:
                    logging.warning('no boxes-like label in roidb, skip this roi')
                    continue
            for i in range(num_boxes):
                new_roi = dict()
                for key in roi:
                    if isinstance(roi[key], (str, float, int, np.float32, np.float64)):
                        new_roi[key] = roi[key]
                        ...
                        try:
                            new_roi[key] = np.array(roi[key][i], dtype=roi[key].dtype)    # noqa
                        except IndexError as e:
    ...
    def roiwise_to_imgwise(roidb, log_level=0):
            ...
            if image_name not in roidb_dict:
                ...
                for key in roi:
                    if isinstance(roi[key], (str, float, int)):
                        new_roi[key] = roi[key]
                    elif isinstance(roi[key], (np.float32, np.float64, np.int64, np.int32)):
                        new_roi[key] = np.array([roi[key]], dtype=roi[key].dtype)    # noqa
                    elif isinstance(roi[key], (np.ndarray)):
                        if roi[key].ndim > 1:
                            new_roi[key] = roi[key]
                        else:
                        ...
                        if log_level >= 1:
                            logging.warning('ignore list, key: %s' % key)
                    else:
                ...
                for key in roi:
                    if isinstance(roi[key], (np.float32, np.float64, np.int64, np.int32)):
                        cur_roi[key] = np.concatenate((
                                        cur_roi[key],
                                        np.array([roi[key]], dtype=roi[key].dtype)), axis=0)
                    elif isinstance(roi[key], (np.ndarray)):
        ...
        for roi in new_roidb:
            for key in roi:
                if isinstance(roi[key], (np.ndarray)) and \
                        roi[key].ndim == 2 and roi[key].shape[1] == 1:
    ...
    def filter_roi(roi, valid):
        ...
        new_roi = dict()
        for key in roi:
            if isinstance(roi[key], (str, float, int, np.float32, np.float64, np.int64, np.int32)):  # noqa
                new_roi[key] = roi[key]
            elif isinstance(roi[key], np.ndarray):
                new_roi[key] = roi[key][valid]
            elif isinstance(roi[key], list):
                new_roi[key] = np.array(roi[key])[valid].tolist()
            else:
                raise ValueError("Unknown data type: %s" % type(roi[key]))
        return new_roi
...
def load_aiottxt(txtfile, field, imgdir, xywh=False):
    with open(txtfile, "r") as fid:
        roidb = aiottxt_to_roidb(
            fid, field,
            imgdir=imgdir,
            xywh=xywh,
        )

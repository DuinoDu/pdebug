...
class Tracklets:
    ...
    def __str__(self):
        ...
        if 'boxes' in _tracklets[0]:
            _str += "\nmin tracklet len: %d" % min([len(x['boxes'])
                                                    for x in _tracklets])
            _str += "\nmax tracklet len: %d" % max([len(x['boxes'])
                                                    for x in _tracklets])
        return _str
    ...
    def update(self, trackid, **kwargs):
        ...
        for key in kwargs:
            if key == 'action' and self.action_dict:
                ...
                tracklet.setdefault('action_label', []).append(self.action_dict[action_label])  # noqa
    ...
    def create(self, trackid, bbox, keypoint=None, action=None):
        ...
        if action and self.action_dict:
            action_label = action[0]
            action_score = float(action[1])
            self.tracklets[trackid]['action_label'] = [self.action_dict[action_label]]  # noqa
    ...
    def insert(self, trackid, bbox, keypoint=None, action=None):
        ...
        """
        print('Please use update interface')
        assert trackid in self.tracklets.keys(), "trackid not in self.tracklets.keys()"  # noqa
    ...
    def generate_action_timestamp(self):
        ...
        for key in self.tracklets:
            ...
            for i in range(starts.shape[0]):
                score = np.array(self.tracklets[key]['action_score'][starts[i]:ends[i]]).mean()  # noqa
...
def tracklets_to_roidb(tracklets: Tracklets, roidb: List[Dict]=0) -> List[Dict]:
    """

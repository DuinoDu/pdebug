...
def simpletxt_to_roidb(fid, field, ignore_nobox=True, imgdir=None, xywh=False, ext='.jpg', log_level=1):
    """
    ...
    for ind, line in enumerate(lines):
        ...
        if len(items) < 5:
            if ignore_nobox:
                ...
                logging.info("find len(line) < 5, invalid aiottxt, please recheck.")  # noqa
                import sys
                sys.exit()
        roi = dict()
        ...
        while True:
            for each_field in field_dict.items():
                ...
                assert len(items) >= (item_ind + seg_len), "items[%d] should > item_ind+seg_len[%d]" % (len(items), item_ind+seg_len)  # noqa
                try:
                    seg_value = [float(x)
                                 for x in items[item_ind: item_ind + seg_len]]
        ...
        if 'boxes' in roi:
            if roi['boxes'].shape[1] > 4:
                ...
                roi['boxes_score'] = [float(1.0)
                                      for _ in range(roi['boxes'].shape[0])]
    ...
    for ind, roi in enumerate(roidb):
        ...
        for box_ind, box in enumerate(boxes):
            ...
            if skeleton:
                ...
                keypoint = roi[name_mapping['skeleton']][box_ind][:skeleton_num]
                for k in keypoint:
                    line += " %.3f" % k
        fid.write(line + '\n')
    fid.close()

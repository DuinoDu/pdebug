...
__all__ = ['minmax_to_0_255', 'unnormalize', 'heatmap', 'dump_batch_and_heatmap']
...
def minmax_to_0_255(data, axis=(2,3)):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        data = np.true_divide(np.subtract(data, min_value),
                              np.subtract(max_value, min_value))
        data = np.nan_to_num(data)
    return data * 255


def unnormalize(data, mean=(128, 128, 128), std=(1./128, 1./128, 1./128),
                isrgb=False, isyuv=False,
                ishwc=False):
    ...
    for i in range(3):
        if normal_method == 'pytorch':
            if unnormal_inside_255:
                new_data[:, :, :, i] = (new_data[:, :, :, i] * std[i] + mean[i]) * 255.
            else:
                new_data[:, :, :, i] = new_data[:, :, :, i] * 255. * std[i] + mean[i]
        else:
...
def heatmap(data, image=None, merge=False, drawscore=False, save=None,
            extra_label=None, no_text=False):
    ...
    if image is not None:
        ...
        for i in range(batch_size):
            for j in range(ch):
                data_resize[i, :, :, j] = cv2.resize(data[i,:,:,j], (img_w, img_h))
        data = data_resize
        batch_size, h, w, ch = data.shape
    ...
    if merge:
        ...
        multi = np.concatenate([data[:,:,:,x] for x in range(ch)], axis=2)  # [b, h, w*ch]
        data = multi
        if image is not None:
            image_ = np.concatenate([image_ for _ in range(ch)], axis=2)      # [b, h, w*ch, 3]
    ...
    if image is not None:
        for batch_ind in range(batch_size):
            ...
            this_data_color = cv2.applyColorMap(this_data.astype(np.uint8), cv2.COLORMAP_JET)
            ...
            if not merge:
                ...
                for i in range(ch):
                    ...
                    res[batch_ind, :, i*w:(i+1)*w, :] = \
                            cv2.putText(res_seg, str_draw, (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
                    ...
                    if extra_label is not None:
                        ...
                        str_draw = str_draw[:-3]
                    res[batch_ind, :, i*w:(i+1)*w, :] = \
                            cv2.putText(res_seg, str_draw, (10, 60), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
    else:
        res[:, :, :, 0] = data[:, :, :]
        res[:, :, :, 1] = data[:, :, :]
        res[:, :, :, 2] = data[:, :, :]
...
def dump_batch_and_heatmap(data, config, output='cache'):
    ...
    batch = data[0]
    imgs = unnormalize(batch.asnumpy(), \
    ...
    # vis kps heatmap
    preds = heatmap(kps_label_pred.asnumpy(), image=imgs, merge=False, drawscore=True)
    ...
    if len(data) > 4:
        ...
        paf_loss = data[6]
        # vis paf heatmap
        preds_paf = heatmap(paf_pred.asnumpy(), image=imgs, merge=False, drawscore=True)
        ...
        for ind in range(len(preds)):
            ...
            max_ind_0, score_0 = argmax2d(kps_label_pred[ind][0].asnumpy(), with_max=True)
            max_ind_1, score_1 = argmax2d(kps_label_pred[ind][1].asnumpy(), with_max=True)
            ...
            if score_0 > score_thres or score_1 > score_thres:
                ...
                valid_point = None
            paf_mod = np.power(paf_pred[ind][0].asnumpy(), 2) + \
                      np.power(paf_pred[ind][1].asnumpy(), 2)
            ...
            if valid_point is None or paf_x.shape[0] == 0:
                ...
                paf_x = paf_x.mean()
                paf_y = paf_y.mean()
                line_paf = Line(p0=valid_point, unit_vec=(paf_x, paf_y), scale=16.0)
    ...
    for ind in range(len(preds)):
        ...
        if has_paf:
            _save_paf = np.concatenate((preds_paf[ind], labels_paf[ind]), axis=0)
            ...
            _save_line = cv2.putText(_save_line.copy(), 'heatmap', (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,255), 2)
            _save_line = cv2.putText(_save_line.copy(), 'paf', (10, 70), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
        ...
        if has_multibin:
            ...
            _pred_img = cv2.putText(_pred_img, "bin: %.1f, %.1f" % (bin_pred[ind][0], bin_pred[ind][1]),
                    (10, 60), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
            _pred_img = cv2.putText(_pred_img, "res: %.3f, %.3f" % (res_pred[ind][0], res_pred[ind][1]),
                    (10, 100),cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
            _pred_img = cv2.putText(_pred_img, "angle: %f" % angle_pred[ind],
                    (10, 140),cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)

            _label_img = _pad_img.copy()
            _label_img = cv2.putText(_label_img, "bin: %d, %d" % (bin_label[ind][0], bin_label[ind][1]),
                    (10, 60), cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,255), 2)
            _label_img = cv2.putText(_label_img, "res: %.3f, %.3f" % (res_label[ind][0], res_label[ind][1]),
                    (10, 100),cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,255), 2)
            _label_img = cv2.putText(_label_img, "angle: %f" % angle_label[ind],
                    (10, 140),cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,255), 2)

            _save_multibin = np.concatenate((_pred_img, _label_img), axis=0)
            _save = np.concatenate((_save, _save_multibin), axis=1)


        savefile = os.path.join(output, '%.3f_image_%03d_%s.jpg' % (time.time(), ind, loss_str))

...
def vis_depth(
    ...
    for batch_idx in range(batch_size):
        ...
        if pred is not None:
            ...
            vis_pred_i = draw.depth(pred_i, image=image_i, vertical=False, hist=True, min_depth=min_depth)
            vis_pred_i = cv2.putText(vis_pred_i, "pred",(100, 100),cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
            vis_list.append(vis_pred_i)
        if target is not None:
            target_i = target[batch_idx]
            vis_target_i = draw.depth(target_i, image=image_i, vertical=False, hist=True, min_depth=min_depth)
            vis_target_i = cv2.putText(vis_target_i, "gt",(100, 100),cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
        ...
        if loss is not None:
            vis_result = cv2.putText(vis_result, f"loss: {loss[batch_idx]: .4f}",(100, 150),cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)

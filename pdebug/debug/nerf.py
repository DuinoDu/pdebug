...
from pdebug.data_types import (
    Camera, CameraExtrinsic, PointcloudTensor, RayTensor, Tensor, x_to_ndarray
)
...
def _serve(js_code: str, *, scale: float, block: bool,
           port: int, vr: bool, title: str, res_dir: str,
...
def wait(port: int = 6151):
    ...
    assert _global_engines_process, (
...
def vis_pose(
    ...
    *,
    ...
    _serve(js_code, scale=scale, block=block, port=port,
           vr=vr, title=title, res_dir=res_dir, no_server=no_server)

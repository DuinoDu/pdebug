...
class Checkpoint:
    ...
    def __init__(self, ckpt_file: str):
        ...
        self.checkpoint = torch.load(ckpt_file, map_location=torch.device('cpu'))
    ...
    def compare(self, item, output="tmp_compare_result", save_hist=False, print_func=print):
        if self == item:
    ...
    def __isub__(self, name: Union[str, List[str]]):
        ...
        if isinstance(name, str):
            name = [name]
        for name_i in name:
            self.state_dict = {k: self.state_dict[k] for k in self.state_dict.keys() if name_i not in k}
        return self
    ...
    def __eq__(self, item):
        if len(self.state_dict) != len(item.state_dict):
            print(f"different state_dict length ({len(self.state_dict)} != {len(item.state_dict)})")
            return False

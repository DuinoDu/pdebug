"""A ``Pipeline`` is a collection of ``Node`` objects.

Pipeline can be executed as a Directed Acyclic Graph, sequentially or in parallel.
The ``Pipeline`` class offers quick access to input dependencies, produced outputs and execution order.
"""
import copy
import json
...
from pdebug.pdag.pipeline.node import Node, _to_list
...
class Pipeline:  # pylint: disable=too-many-public-methods
    ...
    This class treats nodes as part of a graph representation and provides inputs,
    ...
    def __init__(
        ...
        *,
        ...
        ::
            ...
            >>> from pdebug.pdag.pipeline import node
            >>>
    ...
    def __repr__(self):  # pragma: no cover
        ...
        nodes_reprs = [
            repr(node) for node in self.nodes[:max_nodes_to_display]
        ]
    ...
    def _remove_intermediates(self, datasets: Set[str]) -> Set[str]:
        ...
        return {
            d for d in datasets if _strip_transcoding(d) not in intermediate
        }

    def inputs(self) -> Set[str]:  # noqa: D400, D205
    ...
    def outputs(self) -> Set[str]:  # noqa: D400, D205
    ...
    def data_sets(self) -> Set[str]:  # noqa: D400, D205
    ...
    def describe(self, names_only: bool = True) -> str:  # noqa: D400, D205
        ...
        def set_to_string(set_of_strings):
            ...
            return (
                ", ".join(sorted(set_of_strings)) if set_of_strings else "None"
            )
    ...
    @property
    def node_dependencies(self) -> Dict[Node, Set[Node]]:  # noqa: D400, D205
    ...
    @property
    def nodes(self) -> List[Node]:  # noqa: D400, D205
    ...
    @property
    def grouped_nodes(self) -> List[List[Node]]:  # noqa: D400, D205
    ...
    def only_nodes(self, *node_names: str) -> "Pipeline":  # noqa: D400, D205
    ...
    def _get_nodes_with_inputs_transcode_compatible(
        ...
        if missing:
            raise ValueError(
                f"Pipeline does not contain data_sets named {missing}"
            )
    ...
    def from_inputs(self, *inputs: str) -> "Pipeline":  # noqa: D400, D205
        """Create a new ``Pipeline`` object with the nodes which depend
        ...
        while next_nodes:
            result |= next_nodes
            outputs = set(
                chain.from_iterable(node.outputs for node in next_nodes)
            )
            starting = outputs
    ...
    def to_outputs(self, *outputs: str) -> "Pipeline":  # noqa: D400, D205
        """Create a new ``Pipeline`` object with the nodes which are directly
        ...
        starting = set(outputs)
        result = set()  # type: Set[Node]
        next_nodes = self._get_nodes_with_outputs_transcode_compatible(
            starting
        )
        ...
        while next_nodes:
            result |= next_nodes
            inputs = set(
                chain.from_iterable(node.inputs for node in next_nodes)
            )
            starting = inputs
    ...
    def from_nodes(self, *node_names: str) -> "Pipeline":  # noqa: D400, D205
        """Create a new ``Pipeline`` object with the nodes which depend
    ...
    def to_nodes(self, *node_names: str) -> "Pipeline":  # noqa: D400, D205
...
def _validate_duplicate_nodes(nodes_or_pipes: Iterable[Union[Node, Pipeline]]):
    ...
    duplicates = defaultdict(
        set
    )  # type: Dict[Union[Pipeline, None], Set[str]]
...
def _validate_transcoded_inputs_outputs(nodes: List[Node]) -> None:
    """Validate transcoded input and outputs.

...
class Node:
    """``Node`` is an auxiliary class facilitating the operations.

    It runs user-provided functions as part of Kedro pipelines.
    """
    ...
    def tag(
        self, tags: Union[str, Iterable[str]]
    ) -> "Node":  # noqa: D400, D205
    ...
    @property
    def inputs(self) -> List[str]:  # noqa: D400, D205
    ...
    def _validate_inputs(self, func, inputs):
        ...
        # unwrapp func if it is partial.
        name = _get_readable_func_name(func)
        if name == "<partial>":
            func = func.func
...
def _dict_inputs_to_list(func: Callable[[Any], Any], inputs: Dict[str, str]):
    """Convert a dict representation of the node inputs to a list.

    Ensuring the appropriate order for binding them to the node's function.
    """
...
def _to_list(
    element: Union[None, str, Iterable[str], Dict[str, str]]
) -> List[str]:

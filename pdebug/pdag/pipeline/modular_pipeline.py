...
from pdebug.pdag.pipeline.pipeline import (
...
def _validate_inputs_outputs(
    ...
    """Validate inputs and outputs of pipeline.
    ...
    if not inputs <= free_inputs:
        raise ModularPipelineError(
            "Inputs should be free inputs to the pipeline"
        )

    if outputs & free_inputs:
        raise ModularPipelineError(
            "Outputs can't contain free inputs to the pipeline"
        )
...
def _get_dataset_names_mapping(
    ...
    Take a name or a collection of dataset names and turn it into a mapping from
    the old dataset names to the provided ones if necessary.
...
def pipeline(
    ...
    *,
    ...
    _validate_datasets_exist(
        inputs.keys(), outputs.keys(), parameters.keys(), pipe
    )

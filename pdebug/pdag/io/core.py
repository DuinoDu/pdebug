...
from pdebug.pdag.utils import load_obj
...
class AbstractDataSet(abc.ABC, Generic[_DI, _DO]):
    ...
    def release(self) -> None:
        ...
        try:
            ...
            message = (
                f"Failed during release for data set {str(self)}.\n{str(exc)}"
            )
            raise DataSetError(message) from exc
...
class AbstractVersionedDataSet(AbstractDataSet[_DI, _DO], abc.ABC):
    ...
    @cachedmethod(
        cache=attrgetter("_version_cache"), key=partial(hashkey, "load")
    )
    def _fetch_latest_load_version(self) -> str:
        ...
        most_recent = next(
            (path for path in version_paths if self._exists_function(path)),
            None,
        )
    ...
    # https://cachetools.readthedocs.io/en/stable/#cachetools.cachedmethod
    @cachedmethod(
        cache=attrgetter("_version_cache"), key=partial(hashkey, "save")
    )
        ...
        if load_version != save_version:
            warnings.warn(
                _CONSISTENCY_WARNING.format(
                    save_version, load_version, str(self)
                )
            )

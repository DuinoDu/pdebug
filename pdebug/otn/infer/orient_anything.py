...
@contextmanager
def _repo_runtime(repo: Path):
    """Run official Orient-Anything code from its repo root."""
...
def main(
    ...
    if not repo.exists():
        raise RuntimeError(f"Orient-Anything repository not found at {repo}")
    ...
    with _repo_runtime(repo):
        ...
        typer.echo(
            typer.style(
                f"loading {len(reader)} images", fg=typer.colors.GREEN
            )
        )
        t = tqdm.tqdm(total=len(reader))
        for rgb in reader:
            t.update()
            vis_result, *angles, conf = infer_func(
                rgb, do_rm_bkg, do_infer_aug
            )

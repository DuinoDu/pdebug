...
import cv2
import numpy as np
from piata.type_cast import boxes_to_points, points_to_boxes
...
class BaseTracker:
    ...
    TRACKER_TYPERS = ['BOOSTING', 'MIL','KCF', 'TLD', 'MEDIANFLOW', 'GOTURN', 'MOSSE', 'CSRT']

    def __init__(self, tracker_type):
        self._tracker_type = tracker_type
        assert tracker_type in self.TRACKER_TYPERS, (
    ...
    def create_tracker(tracker_type: str) -> "cv2.Tracker":
        if tracker_type == 'BOOSTING':
            ...
            raise NotImplementedError(f"Unsupported tracker type: {tracker_type}")
        return tracker
    ...
    def inner_tracker(self) -> List["cv2.Tracker"]:
        if not self._inner_tracker:
            ...
            for bbox in self._boxes:
                self._inner_tracker.append(self.create_tracker(self._tracker_type))
